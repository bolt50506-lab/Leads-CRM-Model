...
def import_leads(p:ImportIn, db:Session=Depends(get_db), u=Depends(current_user)):
    ...
    for i,row in enumerate(p.rows,1):
        ...
        if key in seen: stats['skipped']+=1; stats['errors'].append({'row':i,'message':'Duplicate row in this import'}); continue
        seen.add(key)
        existing=(existing_by_email.get(email) if email else None) or (existing_by_phone.get(phone) if phone else None)
        st=stages.get(str(norm.get('stage','')).strip().lower()) or default_stage
        owner_value=str(norm.get('assigneduser',norm.get('agentid',''))).strip().lower()
        owner=users.get(owner_value)
        if not owner:
        ...
        if existing:
            ...
            existing.last_activity_at=datetime.now(timezone.utc); stats['updated']+=1
            db.add(Activity(id='a_'+secrets.token_hex(8),lead_id=existing.id,user_id=u.id,text='Updated via import'))
            existing_by_email[normalize_email(existing.email)]=existing if normalize_email(existing.email) else existing
            existing_by_phone[normalize_phone(existing.phone)]=existing if normalize_phone(existing.phone) else existing
        else:
